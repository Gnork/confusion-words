...
def token_sequence(sentence, token_embeddings, w2v_embeddings):
    tokens = ['S']+sentence+['E']
    data = []
    targets = []
    for i in range(len(tokens)-1):
        data.append(w2v_embeddings.token_to_vec(tokens[i]))
...
def token_sequence_generator(sentences, token_embeddings, w2v_embeddings):
    for i, sentence in enumerate(sentences):
        yield token_sequence(sentence, token_embeddings, w2v_embeddings)
...
class Word2VecEmbeddings:
    def __init__(self, file_location):
        print('w2v - load embeddings...')
        print()	
        self.embeddings = gensim.models.Word2Vec.load(file_location)
        self.embeddings_size = len(self.embeddings['the'])
        print('w2v - word vector size: ', self.embeddings_size)
        print()
